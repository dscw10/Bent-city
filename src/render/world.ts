...
import type { Chunk, Scenery } from './scenery';
...
/**
 * Owns the three.js side of the game: renderer, scene, whatever scenery the
 * current level built, and the two per-frame geometry batches.
 *
...
export class World {
  ...
  /** The scenery of the place currently loaded. Replaced on a level change. */
  scenery: Scenery = { blocks: [], chunks: [] };
   ...
   *
   * Geometry is disposed rather than just detached: the city is 25 meshes
   * sharing one 100k-vertex buffer and the pass is twenty strips of valley, and
   * bouncing between them a few times while choosing a level would otherwise
   * leave every one of those buffers resident on the GPU. Meshes can share a
   * geometry, so dispose through a set.
   */
  load(level: { build(scene: THREE.Scene): Scenery }): Scenery {
    const geos = new Set<THREE.BufferGeometry>();
    for (const c of this.scenery.chunks) {
      this.scene.remove(c.mesh);
      geos.add(c.mesh.geometry);
    }
    for (const g of geos) g.dispose();

    this.scenery = level.build(this.scene);
    return this.scenery;
  }
  ...
  setFrame(x: number, z: number, lagHeading: number): void {
    ...
    // The height the fold flattens toward. Barely matters in the city; on a
    // pass that climbs 90 metres it is the difference between a map and a hole.
    uniforms.uGroundY.value = terrainAt(x, z);
  }

  /**
   * Switch off chunks that cannot contribute to this frame.
   *
   ...
   * so a chunk entirely behind the truck, or entirely past where fog has faded
   * the world to paper, cannot appear whatever the fold is doing.
   *
   * It knows nothing about how the level is laid out any more — the city's
   * twenty-five tile copies and the pass's strip of valley segments are both
   * just rectangles with a mesh attached.
   */
  cullChunks(carX: number, carZ: number, lagHeading: number): void {
    ...
    for (const t of this.scenery.chunks) {
      if (t.everywhere) { t.mesh.visible = true; continue; }
      let minZ = Infinity, maxZ = -Infinity;
      for (const [wx, wz] of [[t.x0, t.z0], [t.x1, t.z0], [t.x0, t.z1], [t.x1, t.z1]]) {
  ...
  /** How many chunks are actually being drawn. Used by the dev overlay. */
  get visibleChunks(): number {
    return this.scenery.chunks.reduce((n: number, t: Chunk) => n + (t.mesh.visible ? 1 : 0), 0);
  }
